...
Console.WriteLine("Definitions (Game) versions: " + realm.DefinitionVersion);

var failedTerritoryTypes = new List<TerritoryType>();
var failedTerritoryTuple = new List<(TerritoryType, Territory)>();

var allTerritoryTypes = realm.GameData.GetSheet<TerritoryType>();

var allTerritoryTypesCount = allTerritoryTypes.Count!;
var currentTerritoryTypeCount = 0;
Console.WriteLine("Know of " + allTerritoryTypesCount + " Territory Types.");
...
allTerritoryTypes
    ...
    .Select(territory_type =>
    {
        ...
        catch (Exception e)
        {
            ...
            failedTerritoryTypes.Add(territory_type);
            return (null, null)!;
        }
    })
    ...
    .ForEach(territory_tuple =>
    {
        try
        {
            Console.WriteLine($"({currentTerritoryTypeCount}/{allTerritoryTypesCount}) [{(100.0 / allTerritoryTypesCount) * currentTerritoryTypeCount}%]");
            export(territory_tuple);
            currentTerritoryTypeCount++;
        }
        catch (Exception e)
        {
            Console.WriteLine("Failed to export: " + territory_tuple.territory_type.Name + ", " + territory_tuple.territory_type.PlaceName + ", " + territory_tuple.territory_type.ZonePlaceName + ", " + territory_tuple.territory_type.RegionPlaceName);
            Console.WriteLine(e.StackTrace);

            failedTerritoryTuple.Add(territory_tuple);
        }
    });
#endregion

#region Failed
if (failedTerritoryTypes.Count() > 0)
{
    Console.WriteLine("Failed Territory Types:");
    failedTerritoryTypes.ForEach(territory_tuple =>
    {
        Console.WriteLine("> " + territory_tuple.Name + ", " + territory_tuple.PlaceName + ", " + territory_tuple.ZonePlaceName + ", " + territory_tuple.RegionPlaceName);
    });
}

if (failedTerritoryTuple.Count() > 0)
{
    Console.WriteLine("Failed Territory Tuples:");
    failedTerritoryTuple.ForEach(territory_tuple =>
    {
        Console.WriteLine("> " + territory_tuple.Item1.Name + ", " + territory_tuple.Item1.PlaceName + ", " + territory_tuple.Item1.ZonePlaceName + ", " + territory_tuple.Item1.RegionPlaceName);
    });
}
#endregion

#region End
Console.WriteLine("Done! All files should be exported.");
Console.WriteLine("Check for errors above.");
#endregion
...
static void export((TerritoryType territory_type, Territory territory) tuple)
{
    ...
    var baseDirectory = $"{Environment.CurrentDirectory}/export/";
    ...
    if (!System.IO.Directory.Exists($"{mtlDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{mtlDirectory}");
    }
    if (!System.IO.Directory.Exists($"{lightsDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{lightsDirectory}");
    }
    if (!System.IO.Directory.Exists($"{objDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{objDirectory}");
    }
    if (!System.IO.Directory.Exists($"{ddsDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{ddsDirectory}");
    }
    if (!System.IO.Directory.Exists($"{pngDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{pngDirectory}");
    }
    if (!System.IO.Directory.Exists($"{texDirectory}"))
    {
        System.IO.Directory.CreateDirectory($"{texDirectory}");
    }
    ...
    {
        var failedModels = new List<(String?, String?, String?)>();
        ...
        foreach (var sgbGroup in sgbFile.Data.OfType<SaintCoinach.Graphics.Sgb.SgbGroup>())
        {
            ...
            foreach (var mdl in sgbGroup.Entries.OfType<SaintCoinach.Graphics.Sgb.SgbModelEntry>())
            {
                ...
                {
                    failedModels.Add((mdl.Name, filePath, e.Message));
                    continue;
                }
        ...
        {
            Console.WriteLine($"Failed to load model '{failedModel.Item1}' path: '{failedModel.Item2}'; Exception: '{failedModel.Item3}'");
        }
    }
